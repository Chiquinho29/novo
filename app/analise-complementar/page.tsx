...
function AnaliseComplementarContent() {
  ...
  const foundItems = ['17 audio files found', '27 conversations found', '113 photos found', '45 vídeos encontrados', 'Calls found']

  return (
    <main className="analysis-page">
      <header className="result-header"><a className="brand" href="/">Info<span>Checker</span></a><a className="back-button" href="/" aria-label="Back to the home page">← Voltar</a><span className="secure-badge">Secure lookup</span></header>
      <section className="analysis-panel" aria-live="polite">
        ...
        {completed && <><p className="analysis-done">Complementary analysis completed for {phone}.</p><div className="found-items" aria-live="polite"><p className="found-items-title">Items found</p>{foundItems.map((item) => <div className="found-item" key={item}><span aria-hidden="true">!</span>{item}</div>)}</div><section className="analysis-summary" aria-labelledby="analysis-summary-title"><h2 id="analysis-summary-title">ANALYSIS RESULTS</h2><div className="summary-indicators">{['Notificações', 'Interações', 'Ligações', 'Mídias', 'Mensagens de voz'].map((item, index) => <div className="summary-indicator" style={{ '--delay': `${index * 90}ms` } as CSSProperties} key={item}><span aria-hidden="true">✓</span>{item}</div>)}</div><a className="view-results-button" href={`/resultado?phone=${encodeURIComponent(phone)}&pending=true`}>VIEW RESULTS</a></section></>}
      </section>
    </main>
  )
}
