...
export type ApproximateLocation = {
  ...
  latitude: number
  longitude: number
  source: 'Código regional' | 'Código do país' | 'IP'
}
...
export default function ApproximateLocationMap({ location }: { location: ApproximateLocation | null }) {
  if (!location) {
    return <div className="map-unavailable" role="status">Localização não disponível</div>
  }

  return <div className="location-map" aria-label={`Mapa aproximado de ${location.city}, ${location.state}`}>
    <MapContainer center={[location.latitude, location.longitude]} zoom={10} scrollWheelZoom={false}>
      ...
      <Marker position={[location.latitude, location.longitude]} icon={markerIcon}>
        <Popup><strong>{location.city}, {location.state}</strong><br />Coordenadas aproximadas: {location.latitude.toFixed(4)}, {location.longitude.toFixed(4)}</Popup>
      </Marker>
    </MapContainer>
  </div>
}
